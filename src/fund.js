...
async function fundRecipient(web3, options = {}) {
  ...
  if (
    options.relayHubAddress.toLowerCase() ===
    data.relayHub.address.toLowerCase()
  ) {
  ...
  const currentBalance = new web3.utils.BN(
    await relayHub.methods.balanceOf(options.recipient).call()
  );
  if (currentBalance.lt(targetAmount)) {
    const value = targetAmount.sub(currentBalance);
    await relayHub.methods
      .depositFor(options.recipient)
      .send({ value, from: options.from });
    return targetAmount;
  } else {
