...
program
  .option(
    "-n, --ethereumNodeURL <url>",
    "url to the local Ethereum node",
    "http://localhost:8545"
  )
  .option(
    "--relayUrl <url>",
    "url to advertise the relayer (defaults to localhost:8090)"
  )
  .option(
    "-p, --port <port>",
    "port to bind the relayer to (defaults to port defined in url)"
  )
  .option(
    "--relayHubAddress <address>",
    "address to the relay hub (deploys a new hub if not set)"
  )
  .option(
    "-f, --from <account>",
    "account to send transactions from (defaults to first account with balance)"
  )
  .option(
    "--workdir <workdir>",
    "working directory for relayer data (defaults to tmp dir)"
  )
...
if (opts.port && opts.relayUrl) {
  console.error(
    `Cannot set both port and relayUrl options. Please set only one.`
  );
  process.exit(1);
}
...
web3.eth
  ...
  .catch(err => {
    console.error(
      `Could not connect to node at ${program.ethereumNodeURL} (${err}).`
    );
  });
